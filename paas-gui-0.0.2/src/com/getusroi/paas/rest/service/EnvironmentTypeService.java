...
public class EnvironmentTypeService {
	 ...
	 */
	@GET
	@Path("/getAllEnvironmentAndNoOfInstanceCount")
	@Produces(MediaType.APPLICATION_JSON)
	public String getAllEnvironmentAndNoOfInstanceCount(@Context HttpServletRequest request) throws DataBaseOperationFailedException{
		LOGGER.debug(".getAllEnvironmentAndNoOfInstanceCount method of EnvironmentTypeService  ");
		envDao = new EnvironmentDAO();
		List<EnvironmentType> listOfApplication = null;
		JSONArray jsonArray=new JSONArray(); 
		JSONArray jsonaPP=new JSONArray();
		jsonaPP.put("Environment");
		jsonaPP.put("Total Counts");
		jsonArray.put(jsonaPP);
		try{
		HttpSession session=request.getSession(false);
		int userId=(int)session.getAttribute("id");
		listOfApplication = envDao.getAllEnvironmentType(userId);
		List list = null;
		for(int i=0; i< listOfApplication.size(); i++){
			LOGGER.debug("i "+i);
			list = new ArrayList(); 
			String envName=listOfApplication.get(i).getName();
			int serviceCount = i+4;
			
			
			list.add(envName);
			list.add(serviceCount);
			//map.put(applName, serviceCount);
			//LOGGER.debug(">> applName "+applName +" : serviceCount  "+serviceCount );
			jsonArray.put(list);
		}
		}catch(Exception e){
