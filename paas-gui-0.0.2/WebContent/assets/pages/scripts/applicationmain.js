...
app.controller('appCtrl', function($scope,$http, srvShareData) {
	...
	app.controller('appDisplayCtrl', function($scope, srvShareData,$http) {
	  ...
	  $scope.sharedData = srvShareData.getData();
	   ...
	   $scope.getAllServiceByAppsId = function(id) {
		    ...
		    $scope.deleteServiceById = function(ServiceId,appid,instanceId) {
		     	var response = $http.get('/paas-gui/rest/applicationService/deleteServiceById/'+ServiceId+'/'+instanceId);
		     	response.success(function(data){
	...
	app.controller('serviceUpdateCtrl', function($scope, srvShareData,$http) {
		...
		//$scope.service = {env:[]}; uncomment and replace with $scope.service = {};
		//uncomment if environment variable if needed
		//$scope.env = [{envkey:'',envvalue:''}];
		 ...
		 $scope.sharedData = srvShareData.getData();
			...
			$scope.myFunct = function(evt) {
				/*alert("commng to myFunct");
